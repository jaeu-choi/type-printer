...
export class TemplateTypeHandler extends BaseTypeHandler {
  ...
  private extractTemplateParts(
    ...
    if (node && this.isTemplateLiteralNode(node)) {
  ...
  private extractPartsFromNode(
    ...
    if (this.isTemplateLiteralNode(node)) {
      // TypeScript 내부 구조에 안전하게 접근
      const templateNode = node as any;
      const parts: TypeNode[] = [];

      try {
        // head (첫 번째 고정 부분)
        if (templateNode.head && templateNode.head.text !== undefined) {
          parts.push(
        ...
        if (
          templateNode.templateSpans &&
          Array.isArray(templateNode.templateSpans)
        ) {
          for (const span of templateNode.templateSpans) {
            ...
            if (span.type) {
              try {
                ...
                // 타입 변환 실패시 fallback
                parts.push(this.createStringLiteralNode("unknown", context));
              }
            }

            // 고정 부분
            if (span.literal && span.literal.text !== undefined) {
              parts.push(
