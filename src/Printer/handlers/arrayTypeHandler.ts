...
export class ArrayTypeHandler extends BaseTypeHandler {
  ...
  private createArrayNode(
    ...
    // 3. 최종 배열 타입 문자열 생성
    const elementTypeString = context.checker.typeToString(elementType.tsType);
    const finalArrayTypeString = `${elementTypeString}[]`;

    // 4. 메타데이터 생성 (finalTypeString 명시적 설정)
    const metadata = this.createExtendedMetadata(type, node, context, {
      isBuiltin: false,
      analysisMethod: "type-checker",
      finalTypeString: finalArrayTypeString, // 🎯 핵심! 명시적으로 설정
      debug: {
        warnings: [],
      },
      // 🔧 커스텀 배열 정보
