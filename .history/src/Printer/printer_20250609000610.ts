...
import { GenericProcessor } from "./handlers/genericProcessor";
/**
...
// analyzeTypeAlias 메서드 수정
  private analyzeTypeAlias(
    ...
    if (isGenericInstance(aliasDecl.type)) {
      ...
      try {
        ...
        console.warn(`⚠️ Generic processing failed: ${error}, falling back to standard analysis`);
  ...
  private handleGenericResult(
    ...
    if (typeNode.metadata) {
      typeNode.metadata.finalTypeString = this.checker.typeToString(definitionType);
      typeNode.metadata.originalTypeName = name;
      typeNode.metadata.genericInfo = {
        isGeneric: true,
        isInstantiated: true,
        typeParameters: Array.from(genericResult.parameterMappings.keys()),
        resolvedArguments: Array.from(genericResult.parameterMappings.values())
    ...
    console.log(`✅ Generic type analysis completed: ${typeNode.metadata?.finalTypeString}`);
