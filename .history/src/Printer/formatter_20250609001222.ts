...
export class TypeFormatter {
  ...
  // formatResultView 메서드도 수정하여 제네릭 정보 간단 표시
  private formatResultView(structure: TypeNode, typeName: string): string {
    // 제네릭 처리된 경우 간단 표시
    if (structure.metadata?.genericProcessInfo) {
      const originalExpr =
        structure.metadata.genericProcessInfo.originalExpression;
      const finalResult = structure.metadata?.finalTypeString;

      if (originalExpr && finalResult) {
        return `${originalExpr} → ${finalResult}`;
      }
    }
