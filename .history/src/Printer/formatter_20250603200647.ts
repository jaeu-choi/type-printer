...
export class TypeFormatter {
  ...
  private formatTreeStructure(
    ...
    switch (structure.type) {
      ...
      case "mapped":
        console.log("🔥 formatMappedTree 호출됨!", structure);
    ...
    if (iterations.length > 0) {
      ...
      iterations.forEach((iteration, index) => {
        ...
        if (iteration.children && iteration.children.length >= 2) {
          ...
          if (equalIndex !== -1) {
            // 앞부분에서 실제 expression 추출
            const beforeEqual = resultValueRaw.substring(0, equalIndex).trim();

            // 원본 패턴에서 expression 부분 추출
            ...
            if (patternMatch) {
              ...
              // K를 실제 값으로 치환
              expression = valueExpr.replace(
                new RegExp(param, "g"),
                `${constraint.split(" ")[1]}[${param}]`
              );
              // FormShape[K] 형태로 변환
              ...
              expression = beforeEqual.replace(/\s+/g, " ").trim();
            }
