...
import { PrintOptions, AnalyzableKind, TypeInfo } from "./types";
import { TypeFormatter } from "./formatter";
import { TypeNode, TypeCreationContext, createTypeCreationContext } from "./ir";
import {
...
export class TypeAliasPrinter {
  ...
  private analyzeTypeAlias(
    ...
    // === 🐛 디버깅 정보 추가 ===
    console.log(
      "🔍 DEBUG: aliasDecl.type.getText():",
      aliasDecl.type.getText()
    );
    console.log("🔍 DEBUG: aliasDecl.type.kind:", aliasDecl.type.kind);
    console.log(
      "🔍 DEBUG: ts.SyntaxKind.StringKeyword:",
      ts.SyntaxKind.StringKeyword
    );

    const aliasType = this.checker.getTypeFromTypeNode(aliasDecl.type);
    console.log("🔍 DEBUG: aliasType.flags:", aliasType.flags);
    console.log("🔍 DEBUG: ts.TypeFlags.String:", ts.TypeFlags.String);
    console.log(
      "🔍 DEBUG: aliasType.flags & ts.TypeFlags.String:",
      !!(aliasType.flags & ts.TypeFlags.String)
    );
    console.log(
      "🔍 DEBUG: checker.typeToString(aliasType):",
      this.checker.typeToString(aliasType)
    );

    // 핸들러 레지스트리 호출 전 상태 확인
    const registryInfo = globalHandlerRegistry.getRegisteredHandlers();
    console.log("🔍 DEBUG: Registered handlers:", registryInfo);

    // 핸들러 찾기 테스트
    const handler = globalHandlerRegistry.findHandler(
      aliasType,
      aliasDecl.type
    );
    console.log("🔍 DEBUG: Found handler:", handler?.name || "NONE");

    if (handler) {
      console.log(
        "🔍 DEBUG: Handler isApplicable test:",
        handler.isApplicable(aliasType, aliasDecl.type)
      );
    }
    ...
    // === 🐛 결과 디버깅 ===
    console.log("🔍 DEBUG: Created typeNode:", {
      kind: typeNode.kind,
      literal: typeNode.literal,
      name: typeNode.name,
      metadata: typeNode.metadata,
    });
    // === 결과 디버깅 끝 ===
    ...
    console.log("🔍 DEBUG: Final type string:", finalTypeString);
