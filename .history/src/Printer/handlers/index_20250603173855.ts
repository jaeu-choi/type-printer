...
export class TypeStructureCollector {
  ...
  private isFunctionType(type: ts.Type): boolean {
    ...
    // 🚨 keyof 결과나 string literal union은 제외
    if (type.isUnion()) {
      // Union 타입이면서 모든 멤버가 literal이면 함수가 아님
    ...
    if (typeString.startsWith("keyof ")) {
      return false;
    }

    // 🚨 string literal 타입 제외
    ...
    try {
      ...
      // 함수 시그니처가 있고, 매개변수나 반환 타입이 명확히 정의된 경우만 함수로 인정
      return signature.parameters.length >= 0; // 기본적인 시그니처 체크
    } catch (error) {
      return false;
    }
  }

  // 🎯 또는 더 간단하게, 함수 타입 체크 순서를 뒤로 이동:
  // createFinalTypeStructure에서 함수 체크를 객체 체크 이후로 이동
