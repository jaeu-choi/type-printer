// src/handlers/conditionalTypeHandler.ts (Enhanced with Infer Support)
...
export class ConditionalTypeHandler extends BaseTypeHandler {
  ...
  isApplicable(type: ts.Type, node?: ts.TypeNode): boolean {
    return this.isConditionalType(type, node) || this.isInferType(type, node);
  }
  ...
  private createConditionalOrInferNode(
    ...
    // 2. infer 단독 타입인지 확인
    if (this.isInferType(type, node) && !this.isConditionalType(type, node)) {
