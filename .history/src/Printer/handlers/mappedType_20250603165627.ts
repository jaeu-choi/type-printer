...
export class MappedTypeHandler implements TypeHandler {
  ...
  canHandle(node: ts.TypeNode): boolean {
    return ts.isMappedTypeNode(node);
  }
  ...
  private extractMappedProcess(
    ...
    components.push({
      type: "reference",
      name: "[TypeParameter]",
      children: [this.extractTypeParameter(typeParameter, context)],
      metadata: {
    ...
    if (typeParameter.constraint) {
      components.push({
        type: "reference",
        name: "[InType]",
        children: [this.collector.collect(typeParameter.constraint, context)],
        metadata: {
    ...
    if (mappedNode.type) {
      components.push({
        type: "reference",
        name: "[ValueType]",
        children: [this.collector.collect(mappedNode.type, context)],
        metadata: {
    ...
    // 5. Key Remapping (as clause) - TypeScript 4.1+
    if (mappedNode.nameType) {
      components.push({
        type: "reference",
        name: "[KeyRemapping]",
        children: [this.collector.collect(mappedNode.nameType, context)],
        metadata: {
