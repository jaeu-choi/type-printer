...
export class UnionTypeHandler extends BaseTypeHandler {
  ...
  private createUnionNode(
    ...
    // 🐛 디버깅: finalTypeString 확인
    console.log(
      `🐛 DEBUG UnionTypeHandler finalTypeString: "${finalTypeString}"`
    );
    ...
    const metadata = this.createExtendedMetadata(type, node, context, {
      ...
      finalTypeString, // 🔍 이 값이 제대로 설정되는지 확인
      debug: {
        warnings: [],
      },
    ...
    // 🐛 디버깅: 생성된 metadata 확인
    console.log(
      `🐛 DEBUG metadata.finalTypeString: "${metadata.finalTypeString}"`
    );

    // 4. 유니온 TypeNode 생성
    const unionNode = typeNodeFactory.createUnion(memberNodes, metadata);

    // 🐛 디버깅: 최종 TypeNode의 metadata 확인
    console.log(
      `🐛 DEBUG final unionNode.metadata.finalTypeString: "${unionNode.metadata?.finalTypeString}"`
    );
