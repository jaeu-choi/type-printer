...
export class UnionTypeHandler extends BaseTypeHandler {
  ...
  private isComplexType(type: ts.Type): boolean {
    ...
    const callSignatures = type.getCallSignatures().length;

    // 각 플래그 체크
    const hasObjectFlag = !!(flags & ts.TypeFlags.Object);
    const hasUnionFlag = !!(flags & ts.TypeFlags.Union);
    const hasIntersectionFlag = !!(flags & ts.TypeFlags.Intersection);
    const hasCallSignatures = callSignatures > 0;

    const result =
      hasObjectFlag || hasUnionFlag || hasIntersectionFlag || hasCallSignatures;

    // 🔍 boolean일 때만 상세 로그 출력
    if (typeDescription === "boolean") {
      console.log(`🐛 DEBUG isComplexType for boolean:`);
      console.log(`   type.flags: ${flags}`);
      console.log(`   ts.TypeFlags.Boolean: ${ts.TypeFlags.Boolean}`);
      console.log(`   ts.TypeFlags.Object: ${ts.TypeFlags.Object}`);
      console.log(`   ts.TypeFlags.Union: ${ts.TypeFlags.Union}`);
      console.log(`   ts.TypeFlags.Intersection: ${ts.TypeFlags.Intersection}`);
      console.log(`   hasObjectFlag: ${hasObjectFlag}`);
      console.log(`   hasUnionFlag: ${hasUnionFlag}`);
      console.log(`   hasIntersectionFlag: ${hasIntersectionFlag}`);
      console.log(`   callSignatures: ${callSignatures}`);
      console.log(`   hasCallSignatures: ${hasCallSignatures}`);
