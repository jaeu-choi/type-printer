...
export class MappedTypeHandler implements TypeHandler {
  ...
  handle(node: ts.TypeNode, context: TypeCollectionContext): TypeStructure {
    ...
    if (context.depth >= context.maxDepth - 1) {
      return this.createDepthLimitedStructure(node, context);
    }
    console.log(
    ...
    try {
      ...
      if (typeParameter.constraint) {
        try {
          ...
          console.log(
            `⚠️ MappedType constraint analysis failed: ${errorMessage}`
          );
          components.push({
            type: "reference",
            name: "[InType]",
      ...
      if (mappedNode.type) {
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : String(error);
          console.log(
            `⚠️ MappedType value type analysis failed: ${errorMessage}`
          );
          components.push({
            type: "reference",
            name: "[ValueType]",
      ...
      if (mappedNode.nameType) {
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : String(error);
          console.log(
            `⚠️ MappedType name type analysis failed: ${errorMessage}`
          );
        }
      }
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      console.log(`🚨 MappedType process failed: ${errorMessage}`);
      return [
        {
          type: "literal",
