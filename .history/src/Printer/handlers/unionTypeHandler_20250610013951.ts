...
export class UnionTypeHandler extends BaseTypeHandler {
  ...
  private createUnionNode(
    ...
    // Step Final: 유니온 합성
    const typeCheckerResult = context.checker.typeToString(type);

    // 🎯 핵심 수정: 멤버들로부터 직접 유니온 문자열 구성!
    const memberTypeStrings = memberNodes.map(
      (node) =>
        node.metadata?.finalTypeString || node.literal || node.name || "unknown"
    );
    const actualUnionString = memberTypeStrings.join(" | ");

    // 🐛 디버깅: 두 방식 비교
    console.log(`🐛 DEBUG TypeChecker result: "${typeCheckerResult}"`);
    console.log(`🐛 DEBUG Composed union: "${actualUnionString}"`);

    // 타입 별칭인 경우 실제 구조를 사용, 아니면 TypeChecker 결과 사용
    const finalTypeString =
      typeCheckerResult !== actualUnionString &&
      actualUnionString.includes(" | ")
        ? actualUnionString
        : typeCheckerResult;

    console.log(`🐛 DEBUG Final chosen: "${finalTypeString}"`);
