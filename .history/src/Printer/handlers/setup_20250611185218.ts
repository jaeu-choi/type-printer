// src/handlers/setup.ts (Updated - infer moved to ConditionalTypeHandler)
...
import { ConditionalTypeHandler } from "./conditionalTypeHandler"; // 🔄 Updated
import { UnionTypeHandler } from "./unionTypeHandler";
import { IntersectionTypeHandler } from "./intersectionTypeHandler";
import { OperatorTypeHandler } from "./operatorTypeHandler"; // 🚫 No longer handles infer
...
/**
 * 🆕 ConditionalTypeHandler 핸들러 간 상호작용 실제 테스트
 */
    ...
    typeAliases.forEach((alias, index) => {
      ...
      if (isApplicable) {
        const context = {
          ...
          maxDepth: 10,
          referencePath: [],
          options: { expanded: true, includeDebugInfo: true },
        };
