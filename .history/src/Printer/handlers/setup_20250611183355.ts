...
export function initializeBaicsHandlers(): void {
  console.log("Registerd basic typehandler");
}
...
export function testOperatorTypeHandler(): boolean {
  ...
  try {
    ...
    console.log(`\n🔬 Infer operator supported: ${isInferSupported}`);

    if (!isInferSupported) {
      console.warn(
        "⚠️ Infer operator not supported - check OperatorTypeHandler setup"
      );
      return false;
    }

    // 🔥 infer 패턴 테스트
    console.log("\n🧪 Testing infer patterns through OperatorTypeHandler:");
    const inferPatterns = [
    ...
    inferPatterns.forEach((pattern, index) => {
      console.log(`   ${index + 1}. ${pattern}`);
      console.log(`      Status: 🔧 Ready for AST-based detection`);
    });

    // 🔥 실제 TypeScript 연동 준비 상태 확인
    console.log("\n🚀 Ready for real TypeScript integration:");
    ...
    console.log("✅ OperatorTypeHandler (with infer) test passed");
    return true;
  } catch (error) {
    console.error("❌ OperatorTypeHandler (with infer) test failed:", error);
    return false;
  }
}

/**
 * 🔥 핸들러 간 상호작용 테스트 (infer + 다른 핸들러들)
 */
export function testHandlerInteractions(): boolean {
  console.log("\n🔥 Testing handler interactions with infer patterns...");

  try {
    console.log("📋 Complex infer patterns requiring multiple handlers:");
    ...
    const complexPatterns = [
      {
        ...
        description: "Conditional checks function, infer extracts return type",
      },
      {
        name: "Infer + Union",
        ...
        description: "Extract array element type, may produce union",
      },
      {
        name: "Deep Nested Infer",
        pattern:
    ...
    console.log("\n🎯 Handler Interaction Points:");
    console.log("   ├─ ConditionalTypeHandler detects 'T extends U ? X : Y'");
    console.log(
      "   ├─ InferTypeHandler detects 'infer R' within extends clause"
    );
    ...
    console.log(
      "   └─ TemplateTypeHandler parses '`${infer A}-${infer B}`' patterns"
    );

    console.log("\n🔍 Expected IR Data Flow:");
    console.log("   1. 🎯 Primary handler creates initial TypeNode");
    console.log(
      "   2. 🔄 Calls globalHandlerRegistry.createTypeNode() for sub-components"
    );
    console.log("   3. 📝 Accumulates EducationalStep[] from each handler");
    console.log(
    ...
    console.log(
      "   ❓ Do handlers actually call each other via globalHandlerRegistry?"
    );
    console.log(
      "   ❓ Are EducationalStep[] properly accumulated across handlers?"
    );
    console.log(
      "   ❓ Do IntermediateStep[] capture the transformation chain?"
    );
    console.log("   ❓ Is metadata preserved and merged correctly?");
    console.log("   ❓ Can we trace the full handler execution path in IR?");

    console.log("✅ Handler interaction test scenarios prepared");
    console.log(
 ...
 * 🔬 IR 구조 검증 테스트
 */
export function testIRStructureValidation(): boolean {
  console.log("\n🔬 Testing IR structure validation...");

  try {
    console.log("📋 IR Components to Validate:");

    const irComponents = [
      {
        component: "TypeNode.metadata.educationalSteps",
        purpose: "Track learning process across handlers",
        expectedContent: "Step-by-step handler interactions",
      },
      {
        component: "TypeNode.intermediateSteps",
        purpose: "Capture transformation chain",
        expectedContent: "Handler-to-handler data flow",
      },
      {
        component: "TypeNode.metadata.finalTypeString",
        purpose: "Store TypeScript compiler result",
        expectedContent: "Actual expanded type from TypeChecker",
      },
      {
        component: "TypeNode.children",
        purpose: "Recursive TypeNode structure",
        expectedContent: "Sub-components processed by other handlers",
      },
      {
        component: "TypeNode.conditionalInfo",
        purpose: "Conditional type specific data",
        expectedContent: "checkType, extendsType, trueType, falseType",
      },
    ];

    irComponents.forEach((comp, index) => {
      console.log(`\n   ${index + 1}. ${comp.component}`);
      console.log(`      Purpose: ${comp.purpose}`);
      console.log(`      Expected: ${comp.expectedContent}`);
      console.log(`      Status: 🔧 Ready for validation`);
    });

    console.log("\n🎯 Critical Validation Points:");
    console.log(
      "   1. 📊 EducationalStep[] accumulation across multiple handlers"
    );
    console.log("   2. 🔄 IntermediateStep[] transformation tracking");
    console.log("   3. 🧩 TypeNode.children proper nesting and recursion");
    console.log("   4. 🎨 Metadata preservation and merging");
    console.log("   5. 🔗 Handler chain execution order tracking");

    console.log("✅ IR structure validation test prepared");
    return true;
  } catch (error) {
    console.error("❌ IR structure validation test failed:", error);
    return false;
  }
}
// /**
...
//     // 🆕 InferTypeHandler 테스트들 추가
//     testInferTypeHandler(),
//     testInferTypeHandlerRouting(),
//     testInferTypeHandlerPatterns(),
//     testInferTypeHandlerWithTypeScript(),
//     testInferTypeHandlerInternals(), //     // 기존 테스트들 (주석 해제 가능)
