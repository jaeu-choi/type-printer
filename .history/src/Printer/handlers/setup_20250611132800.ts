...
/**
 * 🆕 InferTypeHandler 기본 테스트
 */
export function testInferTypeHandler(): boolean {
  console.log("\n🧪 Testing InferTypeHandler...");

  try {
    const handler = new InferTypeHandler();
    const examples = InferTypeHandler.getSupportedTypes();

    console.log("📋 Infer type examples:");
    examples.forEach((type, index) => {
      console.log(`   ${index + 1}. ${type}`);
    });

    const testExamples = InferTypeHandler.createExamples();
    console.log("📋 Detailed infer examples:");
    testExamples.forEach((example, index) => {
      console.log(`   ${index + 1}. ${example.description}: ${example.value}`);
      console.log(`      Expected pattern: ${example.expectedPattern}`);
      console.log(`      Expected result: ${example.expectedResult}`);
    });

    console.log("✅ InferTypeHandler basic test passed");
    return true;
  } catch (error) {
    console.error("❌ InferTypeHandler test failed:", error);
    return false;
  }
}
