...
export class UnionTypeHandler extends BaseTypeHandler {
  ...
  private createUnionNode(
    ...
    // Step Final: 유니온 합성
    // 🧪 실험: TypeScript 컴파일러 API 다양한 방법들 테스트
    console.log(`🧪 EXPERIMENT: TypeScript API methods for union types`);

    // 방법 1: 기본 typeToString
    const method1 = context.checker.typeToString(type);
    console.log(`   Method 1 (basic): "${method1}"`);

    // 방법 2: typeToString with flags
    const method2 = context.checker.typeToString(
      type,
      node,
      ts.TypeFormatFlags.InTypeAlias
    );
    console.log(`   Method 2 (InTypeAlias): "${method2}"`);

    // 방법 3: typeToString with NoTruncation
    const method3 = context.checker.typeToString(
      type,
      node,
      ts.TypeFormatFlags.NoTruncation
    );
    console.log(`   Method 3 (NoTruncation): "${method3}"`);

    // 방법 4: typeToString with UseFullyQualifiedType
    const method4 = context.checker.typeToString(
      type,
      node,
      ts.TypeFormatFlags.UseFullyQualifiedType
    );
    console.log(`   Method 4 (UseFullyQualifiedType): "${method4}"`);

    // 방법 5: 별칭 vs 실제 타입 확인
    console.log(`   type.aliasSymbol: ${type.aliasSymbol?.name || "none"}`);
    console.log(`   type.symbol: ${type.symbol?.name || "none"}`);

    // 방법 6: 조합 플래그 테스트
    const combinedFlags =
      ts.TypeFormatFlags.InTypeAlias | ts.TypeFormatFlags.NoTruncation;
    const method6 = context.checker.typeToString(type, node, combinedFlags);
    console.log(`   Method 6 (Combined): "${method6}"`);

    // 방법 7: node 없이 시도
    const method7 = context.checker.typeToString(
      type,
      undefined,
      ts.TypeFormatFlags.InTypeAlias
    );
    console.log(`   Method 7 (no node): "${method7}"`);

    // 🎯 최적의 방법 선택 (일단 기본값, 실험 결과 보고 수정)
    const finalTypeString = method1;
