...
export class TypeAliasPrinter {
  ...
  private analyzeTypeAlias(
    ...
    console.log(`   Declaration text: "${aliasDecl.getText()}"`);
    console.log(
      `   Type node kind: ${aliasDecl.type.kind} (${
        ts.SyntaxKind[aliasDecl.type.kind]
      })`
    );
    console.log(`   Type node text: "${aliasDecl.type.getText()}"`);
    console.log(
      `   Is conditional type node: ${ts.isConditionalTypeNode(aliasDecl.type)}`
    );
    // 🆕 제네릭 인스턴스 감지 및 처리
