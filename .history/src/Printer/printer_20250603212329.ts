...
export class TypeAliasPrinter {
  ...
  public printType(name: string, options?: PrintOptions): void {
    ...
    if (mappedAnalysis) {
      mappingInfo = this.collectMappingAnalysisInfo(mappedAnalysis);
    }
    ...
    let mappingInfo = null;
    if (mappedType) {
      mappingInfo = this.collectMappingInfo(mappedType);
    }

    switch (kind) {
      case AnalyzableKind.TYPEALIAS:
        const typeInfo = this.collectTypeAliasInfo(typeName, context);
        const formatted = this.formatter.format(typeInfo, options, mappingInfo); // 🆕 mappingInfo 전달
  ...
  private collectMappingInfo(mappedTypeName: string) {
    const mappedDecl = this.findTypeDeclaration(mappedTypeName);
