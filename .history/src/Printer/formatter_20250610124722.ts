  ...
  switch (structure.kind) {
    ...
    case "array":
      if (structure.elementType) {
        const elementType = this.getSimpleTypeRepresentation(structure.elementType);
        return `${elementType}[]`;
      }
      return "array";
      ...
      if (structure.objectMembers && structure.objectMembers.length > 0) {
        const members = structure.objectMembers
          .map(member => {
            ...
            const valueType = member.node.metadata?.finalTypeString || 
                             member.node.literal || 
                             "unknown";
    ...
    case "reference":
      if (structure.name && structure.typeArguments?.length) {
        const args = structure.typeArguments
          .map(arg => arg.metadata?.finalTypeString || arg.name || "unknown")
          .join(", ");
        return `${structure.name}<${args}>`;
      }
