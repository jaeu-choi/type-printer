...
export class TypeFormatter {
  ...
  private formatExpandedTreeView(
    ...
    const parts: string[] = [];
    console.log("🔍 formatExpandedTreeView 디버깅:");
    console.log("  structure.type:", structure.type);
    console.log("  structure.children 길이:", structure.children?.length || 0);
    console.log("  structure.computedResult 존재:", !!structure.computedResult);
  ...
  private formatTreeStructure(
    ...
    console.log(
      `🔍 formatTreeStructure: type=${structure.type}, depth=${depth}, expanded=${expanded}`
    );
    switch (structure.type) {
      case "primitive":
        console.log("프리미티브 분기");
