...
export class TypeCheckerBasedMappingAnalyzer {
  ...
  private calculateFinalResult(
   ...
   /**
   * 🔥 High Priority: KeyAnalysis를 포맷터용 iteration 구조로 변환
   */
  ...
  private extractFinalKeys(
    ...
    const remappingStep = steps.find(step => step.stepType === "nested-mapping");
  ...
  private calculateFinalKeyResult(
    ...
    try {
      ...
      const valueResult = this.collector.collect(pattern.valueExpression, keyContext);
      
      console.log(`🔍 Final result for key "${key}":`, valueResult.metadata?.finalTypeString);
      
      return valueResult;

    } catch (error) {
      console.log(`⚠️ Failed to calculate final result for key "${key}": ${error}`);
      return null;
    }
  }
  ...
  /**
   * 🟡 Medium Priority: TypeStructure를 ts.Type으로 변환
   */
  private typeStructureToTsType(
    structure: TypeStructure,
    context: TypeCollectionContext
  ): ts.Type {
    try {
      // TypeStructure에서 finalTypeString을 가져와서 역으로 ts.Type 생성
      const typeString = structure.metadata?.finalTypeString || structure.value || "unknown";
      
      // 간단한 구현: factory로 타입 노드 생성 후 TypeChecker로 계산
      const factory = ts.factory;
      
      // 기본 타입들 처리
      if (typeString === "string") {
        return context.checker.getStringType();
      }
      if (typeString === "number") {
        return context.checker.getNumberType();
      }
      if (typeString === "boolean") {
        return context.checker.getBooleanType();
      }
      
      // 복잡한 타입의 경우 TypeReference 생성
      const typeRefNode = factory.createTypeReferenceNode(typeString);
      return context.checker.getTypeFromTypeNode(typeRefNode);
      
    } catch (error) {
      console.log(`⚠️ Failed to convert TypeStructure to ts.Type: ${error}`);
      // Fallback: any 타입 반환
  ...
  /**
   * 🟡 Medium Priority: 중첩 매핑 정보 추출
   */
  private extractNestedMappingInfo(branchNode: ts.TypeNode): any | null {
    try {
      // 중첩 매핑이 있는지 확인
      // 예: { [K2 in keyof T[K] as `${K}.${K2}`]: T[K][K2] }
      
      if (ts.isMappedTypeNode(branchNode)) {
        return {
          isNestedMapping: true,
          iteratorVar: branchNode.typeParameter?.name.text,
          constraint: branchNode.typeParameter?.constraint?.getText(),
          valueExpr: branchNode.type?.getText(),
          hasKeyRemapping: !!branchNode.typeParameter?.constraint // 임시로 constraint 존재 여부로 판단
        };
      }
      
      return null;
    } catch (error) {
      console.log(`⚠️ Failed to extract nested mapping info: ${error}`);
      return null;
    }
  }

  /**
   * 🟢 Low Priority: 분석 결과 검증
   */
  private validateKeyAnalysisAgainstFinalResult(
    keyAnalyses: KeyAnalysis[],
    finalResult: FinalMappingResult
  ): void {
    try {
      console.log("🔍 Validating key analysis against final result...");
      
      // 분석된 키들과 최종 결과의 프로퍼티들이 일치하는지 확인
      const analyzedKeys = keyAnalyses.flatMap(analysis => analysis.finalKeys);
      const resultProperties = finalResult.properties.map(prop => prop.name);
      
      console.log(`🔍 Analyzed keys: [${analyzedKeys.join(", ")}]`);
      console.log(`🔍 Result properties: [${resultProperties.join(", ")}]`);
      
      // 간단한 검증: 키 개수 비교
      if (analyzedKeys.length !== resultProperties.length) {
        console.log(`⚠️ Key count mismatch: analyzed=${analyzedKeys.length}, result=${resultProperties.length}`);
      } else {
        console.log(`✅ Key analysis validation passed`);
      }
