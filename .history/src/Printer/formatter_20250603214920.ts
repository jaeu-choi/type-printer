...
export class TypeFormatter {
  ...
  format(info: TypeInfo, options?: PrintOptions): string {
    ...
    if (expanded) {
      // 🆕 매핑 분석 모드 체크
      if (options?.mappedAnalysis?.enabled) {
  ...
  private formatMappingAnalysis(
    ...
    // 🆕 여기서 mappedAnalysis.pattern과 typeArgs를 사용해서
    // 매핑 정보를 수집하고 이터레이션을 계산해야 함

    parts.push(`┌─ [Mapped from]: ${mappedAnalysis.pattern}`);
    parts.push("├─ [Pattern]: [K in keyof T]: F[T[K]]"); // 임시
    parts.push("└─ [Final Result]");

    // 최종 결과 표시
    const resultContent = this.formatTreeStructure(
      structure.computedResult || structure,
      false,
      1
    );
