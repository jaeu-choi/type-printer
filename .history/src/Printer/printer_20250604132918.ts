...
export class TypeAliasPrinter {
  ...
  private simulateMappingIterations(
    ...
    try {
      ...
      // 최종 결과가 객체 타입인 경우에만 이터레이션 시뮬레이션
      const properties = finalType.getProperties();
      if (!properties || properties.length === 0) {
        console.log("⚠️ Final type has no properties to iterate");
        return [];
      }

      // 🎯 각 프로퍼티에 대해 매핑 과정 시뮬레이션
      for (const prop of properties) {
        ...
        iterations.push({
          ...
          children: [
            {
              type: "literal",
              value: `${iteratorVar} = "${propName}"`,
              metadata: { description: "Iterator variable value" },
            },
            {
              type: "literal",
              value: `${valueExpr} = ${propTypeString}`,
