...
import { TypeNode, TypeNodeKind, ObjectMember, EducationalStep } from "./ir";
...
export class TypeFormatter {
  ...
  // 1. formatResultView 메서드 개선 (expanded=false일 때)
  private formatResultView(structure: TypeNode, typeName: string): string {
    // 🎯 개선: finalTypeString 최우선 사용 (실제 확장된 타입 표시)
    ...
    switch (structure.kind) {
      case "union":
        if (structure.children) {
          return structure.children
            .map(
              (child) =>
                child.metadata?.finalTypeString ||
                child.literal ||
                child.name ||
                "unknown"
            )
      ...
      case "array":
        if (structure.elementType?.metadata?.finalTypeString) {
          return `${structure.elementType.metadata.finalTypeString}[]`;
        }
        break;

      case "reference":
        if (structure.name && structure.typeArguments?.length) {
          const args = structure.typeArguments
            .map(
              (arg) => arg.metadata?.finalTypeString || arg.name || "unknown"
            )
            .join(", ");
  ...
  // 3. 새로운 헬퍼 메서드: 간단한 타입 표현 생성
  private getSimpleTypeRepresentation(structure: TypeNode): string {
    switch (structure.kind) {
      case "union":
        if (structure.children) {
          return structure.children
      ...
      case "array":
        if (structure.elementType) {
          const elementType = this.getSimpleTypeRepresentation(
      ...
      case "object":
        if (structure.objectMembers && structure.objectMembers.length > 0) {
          const members = structure.objectMembers
            .map((member) => {
      ...
      case "reference":
        ...
        return structure.name || "reference";

      default:
        return structure.literal || structure.name || structure.kind;
    }
  }
  private formatTreeStructure(
    ...
    switch (structure.kind) {
      ...
      case "reference":
        // 🎯 개선: finalTypeString을 우선 사용, 익명 타입 처리
        let refName = structure.name || "unknown";

        // 익명 타입(__type)인 경우 finalTypeString 사용
        if (refName === "__type" && structure.metadata?.finalTypeString) {
          refName = structure.metadata.finalTypeString;
        }
  ...
  private formatUnionTree(
    ...
    // 🎯 개선: expanded=false일 때는 finalTypeString 또는 간단한 형태
    if (!expanded) {
      if (structure.metadata?.finalTypeString) {
        return structure.metadata.finalTypeString;
      }
    ...
    // expanded=true일 때는 기존 트리 형태
    if (!structure.children || structure.children.length === 0) {
      return structure.metadata?.finalTypeString || "union";
    }
  ...
  // 현재 formatter.ts의 formatExpandedTreeView 메서드를 이것으로 교체하세요:
  ...
  private formatExpandedTreeView(
    ...
    // 🆕 Operator 타입 분석 과정 표시 (keyof, typeof 등)
    if (structure.kind === "operator" && structure.metadata?.educationalSteps) {
      parts.push("┌─ [Operator Analysis Process]");
      ...
      structure.metadata.educationalSteps.forEach((step, index) => {
        ...
        if (step.details && typeof step.details === "object") {
          ...
          detailKeys.forEach((key, detailIndex) => {
            ...
            let value = step.details[key];
            // 배열인 경우 깔끔하게 표시
            if (Array.isArray(value)) {
              value = value.join(", ");
            }

            parts.push(`│   ${prefix}  ${detailConnector} ${key}: ${value}`);
          });
        }
      });
      parts.push("│");
    }
    // 🆕 Union 타입 분석 과정 표시 (union 타입 전용)
    else if (
      structure.kind === "union" &&
      structure.metadata?.educationalSteps
    ) {
      parts.push("┌─ [Union Type Analysis Process]");
    ...
    // 🆕 제네릭 교육적 정보 표시 (실제 제네릭인 경우만)
    else if (
      structure.metadata?.educationalSteps &&
      structure.metadata?.genericInfo?.isGeneric
    ) {
    ...
    // 🎯 핵심 수정: Result 섹션에서 실제 확장된 타입 표시
    parts.push("└─ [Result]");

    // 🔥 Intersection 타입의 expanded 모드 특별 처리
    if (
      structure.kind === "intersection" &&
      structure.metadata?.finalTypeString
    ) {
      const expandedResult = this.generateExpandedIntersectionResult(structure);
      console.log(`🎯 Intersection expanded result: "${expandedResult}"`);
      parts.push(`    ${expandedResult}`);
    }
    // 🔥 핵심: finalTypeString을 최우선으로 사용
    else if (structure.metadata?.finalTypeString) {
      console.log(
        `🎯 Using finalTypeString: "${structure.metadata.finalTypeString}"`
      );
      parts.push(`    ${structure.metadata.finalTypeString}`);
    }
    // Union 타입 특별 처리
    else if (structure.kind === "union" && structure.children) {
      const unionTypes = structure.children
        .map((child) => {
          return (
            child.metadata?.finalTypeString ||
            child.literal ||
        ...
        .join(" | ");
      console.log(`🎯 Union fallback: "${unionTypes}"`);
      parts.push(`    ${unionTypes}`);
    }
    // 최종 fallback
    else {
      console.log(`🎯 Final fallback for: ${structure.kind}`);
  ...
  /**
   * 🆕 Intersection 타입의 확장된 결과 생성
   */
  private generateExpandedIntersectionResult(structure: TypeNode): string {
    if (structure.kind !== "intersection" || !structure.children) {
      return structure.metadata?.finalTypeString || "intersection";
    }

    // 모든 멤버가 객체 타입인지 확인
    const allObjectMembers = structure.children.every(
      (child) =>
        child.kind === "reference" &&
        child.metadata?.finalTypeString?.startsWith("{")
    );
    ...
    structure.children.forEach((child) => {
      if (child.kind === "reference" && child.metadata?.finalTypeString) {
        const typeString = child.metadata.finalTypeString;
        // "{ name: string; }" 에서 "name: string" 추출
        const memberMatch = typeString.match(/{\s*(.+?)\s*}/);
        if (memberMatch) {
          // 🔧 세미콜론 제거하고 깔끔하게 정리
          const cleanMember = memberMatch[1].trim().replace(/;+$/, ""); // trailing 세미콜론 제거
          mergedMembers.push(cleanMember);
        }
      }
    });
  ...
  //   // 1. finalTypeString 우선 사용 (실제 확장된 타입 표시)
  //   if (structure.metadata?.finalTypeString) {
  //     parts.push(`    ${structure.metadata.finalTypeString}`);
  //   }
  //   // 2. 특별한 경우: union 타입의 간단한 표시
  //   else if (structure.kind === "union" && structure.children) {
  //     const unionTypes = structure.children
  //       .map((child) => {
  //         return (
