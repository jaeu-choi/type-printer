...
export class TypeAliasPrinter {
  ...
  private simulateMappingIterations(
    ...
    try {
      ...
      if (typeArgDefinitions.length < 1) {
        console.log("⚠️ Need at least 1 type argument for mapping simulation");
        return [];
      }

      // 단일 타입 인자인 경우 (Flatten<Nested>)
      if (typeArgDefinitions.length === 1) {
        const typeInfo = this.analyzeTypeArgument(typeArgDefinitions[0].name);
        if (!typeInfo) {
          console.log("⚠️ Failed to analyze type argument");
          return [];
        }

        console.log("🔍 Single type analysis:", {
          props: typeInfo.properties?.length || 0,
        });

        // 단일 타입의 각 키에 대해 매핑 과정 시뮬레이션
        if (typeInfo.properties) {
          for (const prop of typeInfo.properties) {
            const keyName = prop.name; // "user", "active"
            const keyType = prop.typeString; // "{id: number, name: string}", "boolean"
            ...
            iterations.push({
              ...
              children: [
                {
                  type: "literal",
                  value: `${iteratorVar} = "${keyName}"`,
                  metadata: { description: "Iterator variable value" },
                },
                {
                  type: "literal",
                  value: `T[${iteratorVar}] = ${keyType}`,
                  metadata: { description: "Key lookup result" },
                },
                {
                  type: "literal",
                  value: `Condition: ${keyType} extends object ? ${this.isObjectType(
                    keyType
                  )}`,
                  metadata: { description: "Conditional type evaluation" },
                },
              ],
              metadata: {
                description: `Mapping iteration for key "${keyName}"`,
                keyValue: keyName,
                keyType: keyType,
              },
            });
      ...
      // 두 개 타입 인자인 경우 (TranslateKeys<From, To>)
      else if (typeArgDefinitions.length === 2) {
        const fromTypeInfo = this.analyzeTypeArgument(
          typeArgDefinitions[0].name
        );
        const toTypeInfo = this.analyzeTypeArgument(typeArgDefinitions[1].name);
        ...
        if (toTypeInfo.properties) {
          for (const toProp of toTypeInfo.properties) {
            ...
            const resultType = fromProp.typeString;
  ...
  // 🆕 객체 타입 판별 헬퍼
  private isObjectType(typeString: string): boolean {
  ...
  private collectTypeAliasInfo(
    ...
    const finalType = symbol
      ...
      : null;
    console.log("🔍 FinalType obtained:", !!finalType);
    ...
    // 🆕 추가 디버깅: 타입 자체를 직접 얻어보기
    const directType = this.checker.getTypeFromTypeNode(aliasDecl.type);
    const directTypeString = this.checker.typeToString(directType);
    console.log("🔍 DirectType (from type node):", directTypeString);

    // 🆕 제네릭 인스턴스화 시도
    if (ts.isTypeReferenceNode(aliasDecl.type)) {
      console.log("🔍 Type is a reference, attempting instantiation...");

      // 🎯 핵심: 제네릭 타입과 타입 인자 분리해서 직접 계산
      const typeName = aliasDecl.type.typeName.getText(); // "Flatten"
      const typeArgs = aliasDecl.type.typeArguments; // ["Nested"]

      console.log("🔍 Generic type name:", typeName);
      console.log(
        "🔍 Type arguments:",
        typeArgs?.map((arg) => arg.getText())
      );

      if (typeArgs && typeArgs.length > 0) {
        // 🎯 제네릭 타입 정의 찾기
        const genericTypeDecl = this.findTypeDeclaration(typeName);
        if (genericTypeDecl && ts.isTypeAliasDeclaration(genericTypeDecl)) {
          console.log("🔍 Found generic type declaration");

          // 🎯 타입 인자들을 실제 타입으로 해석
          const resolvedArgs = typeArgs.map((arg) => {
            const argType = this.checker.getTypeFromTypeNode(arg);
            const argTypeString = this.checker.typeToString(argType);

            // 🆕 타입 인자의 실제 구조도 계산
            let argStructure: TypeStructure | null = null;
            try {
              // 타입 인자가 사용자 정의 타입인 경우 구조 분석
              const argTypeName = arg.getText();
              const argTypeDecl = this.findTypeDeclaration(argTypeName);
              if (argTypeDecl) {
                console.log("🔍 Found type argument declaration:", argTypeName);
                // 타입 인자의 구조를 별도 컨텍스트로 계산
                const argContext = { ...context, depth: 0, referencePath: [] };
                if (ts.isInterfaceDeclaration(argTypeDecl)) {
                  argStructure = this.collectInterfaceStructure(
                    argTypeDecl,
                    argContext,
                    argTypeString
                  );
                } else if (ts.isTypeAliasDeclaration(argTypeDecl)) {
                  argStructure = this.collector.collect(
                    argTypeDecl.type,
                    argContext
                  );
                }
                console.log("🔍 Arg structure calculated:", argStructure?.type);
              }
            } catch (error) {
              console.log("⚠️ Failed to calculate arg structure:", error);
            }

            console.log("🔍 Resolved arg:", arg.getText(), "→", argTypeString);
            return {
              node: arg,
              type: argType,
              typeString: argTypeString,
              structure: argStructure,
            };
          });

          // 🎯 제네릭 컨텍스트 생성 후 직접 계산 시도
          const genericContext = new Map<string, TypeStructure>();
          if (genericTypeDecl.typeParameters) {
            genericTypeDecl.typeParameters.forEach((param, index) => {
              if (index < resolvedArgs.length) {
                const paramName = param.name.text;
                const resolvedArg = resolvedArgs[index];

                // 🆕 실제 구조가 있으면 구조를 사용, 없으면 타입 문자열 사용
                const contextValue = resolvedArg.structure || {
                  type: "primitive" as const,
                  value: resolvedArg.typeString,
                };

                genericContext.set(paramName, contextValue);
                console.log(
                  "🔍 Generic context:",
                  paramName,
                  "=",
                  resolvedArg.typeString,
                  resolvedArg.structure ? "(with structure)" : "(string only)"
                );
              }
            });
          }

          // 🎯 수정된 컨텍스트로 제네릭 타입 본체 계산
          ...
          console.log(
            "🔍 Attempting to calculate generic body with context..."
          );
          const structure = this.collector.collect(
          ...
          const resultTypeString =
            structure.metadata?.finalTypeString || "unknown";
          console.log("🔍 Generic calculation result:", resultTypeString);

          if (
            resultTypeString !== "unknown" &&
            !resultTypeString.includes("T")
          ) {
            console.log("✅ Using generic calculation result");
            return {
      ...
      // Fallback: 원래 방식
      const refType = this.checker.getTypeFromTypeNode(aliasDecl.type);
      const instantiatedTypeString = this.checker.typeToString(refType);
      console.log("🔍 Fallback instantiated type:", instantiatedTypeString);

      // 🎯 실제 인스턴스화된 타입 사용
      if (
        instantiatedTypeString !== "any" &&
        !instantiatedTypeString.includes("T") &&
        instantiatedTypeString !== name
      ) {
        console.log("✅ Using instantiated type as final result");
        const structure = this.collector.collect(aliasDecl.type, context);

        // 올바른 최종 타입 문자열 사용
        if (structure.metadata) {
          structure.metadata.finalTypeString = instantiatedTypeString;
        } else {
          structure.metadata = { finalTypeString: instantiatedTypeString };
        }
    ...
    // 🆕 타입 플래그 확인
    if (finalType) {
      console.log("🔍 Type flags:", finalType.flags);
      console.log("🔍 Is union:", finalType.isUnion());
      if (finalType.isUnion()) {
        console.log(
          "🔍 Union types:",
          finalType.types.map((t) => this.checker.typeToString(t))
        );
      }
    }
